...
namespace WaykDen.Models.Services
{
    public class DenLucidService : DenService
    {
        ...
        private const string DEFAULT_DB_URL = "mongodb://den-mongo";
        private const string DEFAULT_PORT = "27017";
        private string ApiKey => this.DenConfig.DenLucidConfigObject.ApiKey;

        public DenLucidService(DenServicesController controller)
        {
            this.DenServicesController = controller;
            this.Name = DENLUCID_NAME;
            this.ImageName = this.DenConfig.DenImageConfigObject.DenLucidImage;

            string dburl = string.IsNullOrEmpty(this.DenConfig.DenMongoConfigObject.Url) ? DEFAULT_DB_URL : this.DenConfig.DenMongoConfigObject.Url;
            dburl = string.IsNullOrEmpty(this.DenConfig.DenMongoConfigObject.Port) ? $"{dburl}:{DEFAULT_PORT}" : $"{dburl}:{this.DenConfig.DenMongoConfigObject.Port}";
